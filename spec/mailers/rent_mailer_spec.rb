...
describe RentMailer do
  ...
  context 'expired_rent' do
    let(:rent) { create(:rent_with_user_and_book) }

    it 'job is created' do
      ActiveJob::Base.queue_adapter = :test
      expect { RentMailer.expired_rent(rent.id).deliver_later }
        .to have_enqueued_job.on_queue('mailers')
    end

    it 'expired_rent is sent' do
      expect { perform_enqueued_jobs { RentMailer.expired_rent(rent.id).deliver_later } }
        .to change { ActionMailer::Base.deliveries.size }.by(1)
    end

    it 'expired_rent is sent to the right user' do
      perform_enqueued_jobs do
